...
export default function WatchlistPage() {
  const navigate = useNavigate();
  const { token, activeProfile, user, removeFromWatchlist: _removeFromWatchlist } = useContext(AuthContext);
  ...
  // 1) Carga perfiles si eres admin, o usa el activo
  useEffect(() => {
    if (user?.role === "admin") {
      api.profiles
        .list(token)
  ...
  // 2) Al cambiar de perfil, cargar detalles
  useEffect(() => {
    if (!selectedProfileId) return;
    setLoading(true);

    api.profiles
      .getWatchlist(selectedProfileId, token) // devuelve ["tt...","tt...", ...]
      .then(ids =>
        Promise.all(
          ids.map(async id => {
            // Primero intento película
            try {
              return await api.movies.get(id, token);
            } catch {
              // Si falla, intento serie
              try {
  ...
  const handleRemove = async itemId => {
    ...
    await _removeFromWatchlist(itemId);
