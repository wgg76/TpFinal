...
router.get("/", requireAuth, async (req, res) => {
  try {
    ...
    const minRating =
      req.query.minRating != null ? parseFloat(req.query.minRating) : NaN;
    const yearFrom =
      req.query.yearFrom != null ? parseInt(req.query.yearFrom, 10) : NaN;
    const yearTo =
      req.query.yearTo != null ? parseInt(req.query.yearTo, 10) : NaN;
...
router.put("/:id", requireAuth, requireAdmin, async (req, res) => {
  try {
    const updated = await Series.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
